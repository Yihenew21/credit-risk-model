...
def load_processed_data(processed_data_dir="data/processed"):
    ...
    if not os.path.exists(processed_data_dir):
        raise FileNotFoundError(
            f"Processed data directory not found at {processed_data_dir}"
        )
    files = os.listdir(processed_data_dir)
    if not files:
        raise FileNotFoundError(
            f"No processed data files found in {processed_data_dir}"
        )
    latest_file = max(
        files,
        key=lambda x: os.path.getmtime(
            os.path.join(processed_data_dir, x)
        ),
    )
...
def train_and_tune_models(X_train, X_test, y_train, y_test, is_test=False):
    """
    Trains and tunes Logistic Regression, Decision Tree, and Random Forest
    models, tracks with MLflow.
    """
        ...
        with mlflow.start_run(run_name=f"{name}_Run"):
            ...
            if name == "LogisticRegression":
                param_grid = {
                    ...
                    "solver": [
                        "lbfgs"
                    ],  # Explicitly set solver to avoid deprecation warning
                }
            ...
            grid_search = GridSearchCV(
                model, param_grid, cv=10, scoring="f1", n_jobs=-1
            )
...
def main():
    ...
    # best_model = train_and_tune_models(

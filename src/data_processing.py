...
class WoETransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        if df.empty or len(df["target"].unique()) < 2:
            logger.warning(
                (
                    f"Insufficient data or target classes for WoE calculation "
        ...
        try:
            ...
            labels, self.bins = pd.qcut(
                ranked_feature, q=self.n_bins, duplicates="drop", retbins=True
            )
            ...
            logger.warning(
                (
                    ...
                    f"'{self.feature_name}' (Error: {ve}). Skipping WoE/IV."
                )
            )
            self.woe_map = {}
            self.iv_value = 0
        ...
        if global_good == 0 or global_bad == 0:
            logger.warning(
                (
                    (
                        f"Global good or bad count is zero for "
                        f"'{self.feature_name}'. WoE/IV cannot be calculated."
                    )
                )
            )
            self.woe_map = {}
    ...
    def transform(self, X):
        ...
        # --- FIX: Convert to float before filling NaNs to avoid
        # Categorical error ---
...
class TemporalFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        logger.info(
            f"Extracting temporal features from '{self.datetime_col}'..."
        )
        X_copy = X.copy()
...
class AggregateFeatureExtractor(BaseEstimator, TransformerMixin):
    """
    A custom transformer to create aggregate features based on CustomerId
    and Amount.
    """
...
class DataProcessor:
    ...
    def build_preprocessor(self):
        """
        Builds the scikit-learn preprocessing pipeline for numerical and
        categorical data.
        """
    ...
    def process_data(self):
        ...
        X = self.df.drop(
            columns=[self.target_col] + self.id_cols,
            errors="ignore"
        )
        ...
        X_processed_df = pd.DataFrame(
            X_processed_array, columns=transformed_feature_names, index=X.index
        )
        ...
        logger.info(
            (
                f"Calculating WoE for features: {self.woe_features} "
                f"with custom transformer..."
            )
        )
    ...
    def save_processed_data(self):
        ...
        logger.info(
            "Processed data saved successfully. Final shape: "
            f"{self.df.shape}"
        )

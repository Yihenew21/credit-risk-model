...
class WoETransformer(BaseEstimator, TransformerMixin):
    """
    A custom transformer to calculate and apply Weight of Evidence (WoE)
    transformation for a specified feature, useful for encoding categorical
    or continuous variables in credit risk modeling.
    ...
    n_bins : int, optional (default=5)
        The number of bins to use for discretizing the feature during WoE
        calculation.
    ...
    def fit(self, X, y):
        """
        Fits the transformer by calculating WoE and IV for each bin of the
        feature.
        ...
        """
        df = pd.DataFrame({
            "feature": X[self.feature_name],
            "target": y
        }).dropna()

        if df.empty or len(df["target"].unique()) < 2:
            logger.warning(
                "Insufficient data or target classes for WoE calculation on "
                f"'{self.feature_name}'."
            )
            self.woe_map = {}
            self.iv_value = 0
        ...
        try:
            ...
            logger.warning(
                (
                    f"Could not create {self.n_bins} bins for "
                    f"'{self.feature_name}' (Error: {ve}). "
                    "Skipping WoE/IV."
                )
            )
            self.woe_map = {}
        ...
        if df["bin"].nunique() < 2:
            logger.warning(
                f"Only {df['bin'].nunique()} bin(s) created for "
                f"'{self.feature_name}'. Skipping WoE/IV."
            )
            self.woe_map = {}
            self.iv_value = 0
            return self
        ...
        if global_good == 0 or global_bad == 0:
            logger.warning(
                f"Global good or bad count is zero for '{self.feature_name}'. "
                "WoE/IV cannot be calculated."
            )
            self.woe_map = {}
            self.iv_value = 0
            return self
        ...
        # Avoid log(0)
        bin_summary["woe"] = np.log(
            bin_summary["good_rate"] / bin_summary["bad_rate"]
        ...
        logger.info(
            "IV for %s: %.4f", self.feature_name, self.iv_value
        )
    ...
    def transform(self, X):
        ...
        if not self.woe_map:  # If WoE calculation failed in fit
            logger.warning(
                (
                    f"WoE mapping not available for '{self.feature_name}', "
                    "assigning 0."
                )
            )
            return pd.DataFrame(
                {f"{self.feature_name}_woe": np.zeros(len(X))}, index=X.index
            )
        ...
        return pd.DataFrame(
            {f"{self.feature_name}_woe": woe_transformed}, index=X.index
        )
...
class TemporalFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        if self.datetime_col not in X.columns:
            raise ValueError(
                (
                    f"Column '{self.datetime_col}' not found in the input "
                    "DataFrame."
                )
            )
        return self
    ...
    def transform(self, X):
        ...
        -------
        pandas.DataFrame
            DataFrame with extracted temporal features (trans_hour, trans_day,
            trans_month, trans_year).
        """
        logger.info(
            "Extracting temporal features from '%s'...", self.datetime_col
        )
        X_copy = X.copy()
        X_copy[self.datetime_col] = pd.to_datetime(
            X_copy[self.datetime_col], errors="coerce"
        )
        ...
        logger.info(
            "Successfully extracted: 'trans_hour', 'trans_day', "
            "'trans_month', 'trans_year'"
        )
...
class AggregateFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        """
        Fits the transformer by calculating aggregate features for each
        customer.
        ...
        if (
            self.customer_col not in X.columns
            or self.amount_col not in X.columns
        ):
            raise ValueError(
                f"Required columns '{self.customer_col}' or "
                f"'{self.amount_col}' not found."
            )
        ...
        logger.info(
            "Aggregate features calculated and stored for transformation."
        )
        return self
    ...
    def transform(self, X):
        ...
        X_copy = X.copy()
        merged_df = X_copy[[self.customer_col]].merge(
            self.agg_features_df, on=self.customer_col, how="left"
        )
...
class DataProcessor:
    ...
    variable (is_high_risk) based on RFM analysis and clustering for credit
    risk modeling.
    ...
    processed_data_path : str, optional
        (default='data/processed/processed_data_task4_{timestamp}.csv')
        Path template for saving the processed data, with {timestamp} replaced
    ...
    def __init__(
        ...
        processed_data_path=(
            "data/processed/processed_data_task4_{timestamp}.csv"
        ),
    ):
        self.raw_data_path = raw_data_path
        self.processed_data_path = processed_data_path.format(
    ...
    def load_data(self):
        ...
        logger.info("Original data shape: %s", str(self.df.shape))
        logger.info("Columns: %s", str(self.df.columns.tolist()))

    def build_preprocessor(self):
        """
        Builds a scikit-learn preprocessing pipeline for numerical and
    ...
    def calculate_rfm(self):
        """
        Calculates Recency, Frequency, and Monetary (RFM) metrics for each
        CustomerId.

        Returns
        -------
        pandas.DataFrame
            DataFrame containing RFM metrics (Recency, Frequency, Monetary)
            for each customer.
        """
        logger.info("Calculating RFM metrics...")
        # Set snapshot date (current date + 1 day, timezone-aware)
        eat = timezone("Africa/Nairobi")
        snapshot_date = pd.to_datetime("2025-07-01 05:13:00").replace(
            tzinfo=eat
        ) + pd.Timedelta(days=1)

        # Convert TransactionStartTime to timezone-aware datetime
        self.df["TransactionStartTime"] = pd.to_datetime(
            self.df["TransactionStartTime"], utc=True
        ...
        logger.info("RFM calculated for %d customers.", len(rfm))
        return rfm
    ...
    def cluster_customers(self, rfm):
        ...
        -------
        tuple
            A tuple (rfm, kmeans) where rfm is the updated DataFrame with
            cluster labels and kmeans is the fitted KMeans model.
        """
        ...
        logger.info(
            "Customers clustered into %d groups.", kmeans.n_clusters
        )
        return rfm, kmeans

    def assign_high_risk_label(self, rfm):
        """
        Analyzes clusters and assigns the high-risk label based on the least
        engaged group.
        ...
        # Analyze cluster characteristics (e.g., low Frequency and low Monetary
        ...
        # Identify the high-risk cluster (highest Recency, lowest Frequency,
        # lowest Monetary)
        ...
        logger.info(
            (
                "High-risk cluster identified as %s. "
                "Assigned 'is_high_risk' labels."
            ),
            str(high_risk_cluster)
        )
        return rfm
    ...
    def integrate_target_variable(self):
        ...
        """
        logger.info(
            "Integrating high-risk target variable into the main dataset..."
        )
        ...
        logger.info(
            "Target variable 'is_high_risk' integrated. New shape: %s",
            str(self.df.shape)
        )

    def process_data(self):
        """
        Executes the full data processing pipeline including proxy target
        variable engineering.

        This method orchestrates the preprocessing of numerical and categorical
        features, extracts temporal and aggregate features, applies WoE
        transformation, and integrates the is_high_risk proxy target variable.
        """
        if self.df is None:
            self.load_data()
        ...
        X = self.df.drop(
            columns=[self.target_col] + self.id_cols, errors="ignore"
        )

        temporal_extractor = TemporalFeatureExtractor(
            datetime_col=self.temporal_col
        )
        temporal_features_df = temporal_extractor.fit_transform(X)

        aggregate_extractor = AggregateFeatureExtractor(
            customer_col=self.customer_col, amount_col="Amount"
        )
        aggregate_features_df = aggregate_extractor.fit_transform(X)

        X = X.drop(
            columns=[self.customer_col, self.temporal_col], errors="ignore"
        )
        ...
        combined_features_df = pd.concat(
            [X_processed_df, temporal_features_df, aggregate_features_df],
            axis=1
        )

        # --- NEW STEP: Apply custom WoE transformation ---
        logger.info(
            "Calculating WoE for features: %s with custom transformer...",
            str(self.woe_features)
        )
        ...
        for feature in self.woe_features:
            ...
            woe_transformed_df = pd.concat(
                [woe_transformed_df, woe_feature_df], axis=1
            )
        ...
        final_df = pd.concat(
            [
                combined_features_df.drop(
                    columns=self.woe_features, errors="ignore"
                ),
                woe_transformed_df,
    ...
    def save_processed_data(self):
        ...
        logger.info(
            "Saving processed data to %s...", self.processed_data_path
        )
        os.makedirs(os.path.dirname(self.processed_data_path), exist_ok=True)
        self.df.to_csv(self.processed_data_path, index=False)
        logger.info(
            "Processed data saved successfully. Final shape: %s",
            str(self.df.shape)
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(
            "An error occurred during data processing: %s", str(e)
        )

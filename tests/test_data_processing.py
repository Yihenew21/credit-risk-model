import pandas as pd
import pytest
from src.model_trainer import load_processed_data, split_data, train_and_tune_models
import mlflow
...
def mock_data():
    data = pd.DataFrame({
        ...
        'is_high_risk': [0, 1] * 25  # Alternating 0s and 1s for balanced classes
    })
    return data
...
def test_split_data_correct_split(mock_data):
    ...
    assert abs(len(X_train) - expected_train_size) <= 1  # Allow for rounding tolerance
...
def test_train_and_tune_models_logs_metrics(mock_data, mocker):
    ...
    mock_start_run = mocker.patch.object(mlflow, 'start_run')
    ...
    mocker.patch.object(mlflow, 'register_model', return_value=mocker.Mock(version="1"))  # Mock registration
